...
if [[ $all -eq 1 ]]; then
	...
	confstr="-n=1;--repair=$owlfile -n=1; --repair=$owlfile -n=1 --replim=5; --repair=$owlfile -n=1 --replim=20; --repair=$owlfile -n=1 --repdel=EDriver" # (2)
	$bmscripts/runconfigs.sh "dlvhex2 --plugindir=../../../../../src --supportsets --liberalsafety --heuristics=monolithic INST CONF" "$confstr" "$instance" "$to"
# (3)
fi
