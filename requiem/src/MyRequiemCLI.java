...
import org.oxford.comlab.requiem.gui.RModel;
...
public class MyRequiemCLI {
	...
	private static final RModel m_model = new RModel();
	/**
	 * @param args
	 * 0 - query file
	 * 1 - ontology file
	...
	public static void main(String[] args) throws Exception{
		...
		//	System.err.println("ontology: " + ontologyURI);
			...
			Clause query = m_parser.getQuery(queryString);
	    	...
	    	ArrayList<Clause> rewriting  = m_model.rewriteQuery(query.toString(), ontologyURI);


	    	long end = System.currentTimeMillis();
			
			printResult(queryString, ontologyFile, end-begin, rewriting.size(), query.m_canonicalRepresentation, rewriting);
	...
	private static void printResult(String queryFile, String ontologyFile, long time, int rewritingSize, String query, ArrayList<Clause> rewriting) throws Exception{
		...
		String s; 
		for(Clause c: rewriting){
			s = c.m_canonicalRepresentation.replace("Q(?0)  <-  ", "");
			s = s.replace("?", "O");
			s = s.replace(", ", " ");
			System.out.print(s + "\n");
		}
